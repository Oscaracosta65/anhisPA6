...
/**
 * --------------------------------------------------------------------------
 * Game configuration — PA6 Pennsylvania Pick 5
 * Change these variables to adapt this page to a different game format
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setDescription('Analyze ' . $stateName . ' ' . $gName . ' draw frequency, hot and cold numbers, recency data, and draw history. Historical results intelligence from LottoExpert.net.');
}
...
$p1 = $latestMainBalls[0] ?? '';
$p2 = $latestMainBalls[1] ?? '';
$p3 = $latestMainBalls[2] ?? '';
$p4 = $latestMainBalls[3] ?? '';
$p5 = $latestMainBalls[4] ?? '';
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $ball, false, $gameBonusCol, $gameMainCols);
...
/**
 * --------------------------------------------------------------------------
 * Structured data (JSON-LD) for SEO / AIO discovery
 * --------------------------------------------------------------------------
 */
$jsonLdPageArr = [
    ...
    'description' => isset($stateName, $gName) ? 'Historical frequency analysis, recency data, and draw history for ' . $stateName . ' ' . $gName . '.' : 'Lottery historical analysis.',
...
$jsonLdDatasetArr = [
    '@context'    => 'https://schema.org',
    '@type'       => 'Dataset',
    'name'        => isset($stateName, $gName) ? $stateName . ' ' . $gName . ' Draw History' : 'Lottery Draw History',
    'description' => isset($stateName, $gName) ? 'Historical draw frequency, recency, and draw dates for ' . $stateName . ' ' . $gName . '. Pick 5 game, numbers 01-30.' : 'Lottery historical draw data.',
    'url'         => $canonicalNoQuery,
    'keywords'    => ['lottery', 'draw history', 'frequency analysis', 'hot numbers', 'cold numbers', 'Pick 5', 'Pennsylvania lottery', 'results'],
];
?>
<script type="application/ld+json"><?php echo json_encode($jsonLdPageArr, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?></script>
<script type="application/ld+json"><?php echo json_encode($jsonLdDatasetArr, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?></script>
<style>
:root{
  --skai-blue:#1C66FF;
  --deep-navy:#0A1A33;
...
.skai-chart-frame--tall{height:680px}
...
@media (max-width:780px){
  ...
  .skai-summary-row{flex-direction:column;gap:4px}
  .skai-summary-label{min-width:unset}
}
        ...
        <div class="skai-logo" aria-hidden="<?php echo $logo['exists'] ? 'false' : 'true'; ?>">
          <?php if ($logo['exists'] && $logo['url'] !== '') : ?>
            <img
              ...
              alt="<?php echo htmlspecialchars((string) ($stateName ?? '') . ' ' . (string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?> lottery logo"
              width="110"
              height="110"
              loading="lazy"
              decoding="async"
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">Draw window currently loaded for the main-number analysis on this page view.</div>
      </div>
    </article>
  </section>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active in the current window, which are quiet, and how the distribution looks at a glance across all <?php echo (int) $gameMainMax; ?> main-number positions.
        </p>
      </div>
    </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current draw easier to interpret. It shows how recently each latest number was previously seen and how the perspective shifts when you compare a short recent window with a broader historical one. Numbers are drawn from a pool of <?php echo htmlspecialchars(lePad2((string) $gameMainMin), ENT_QUOTES, 'UTF-8'); ?>&ndash;<?php echo htmlspecialchars(lePad2((string) $gameMainMax), ENT_QUOTES, 'UTF-8'); ?>.
        </p>
      </div>
    </div>
          ...
          <div class="skai-card-body">
            <div class="skai-history-list">
              <?php if (empty($drawHistoryRows)) : ?>
                <p style="font-size:14px;color:var(--text-soft)">No draw history available.</p>
              <?php else : ?>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The left panel shows the complete main-number distribution across all values <?php echo htmlspecialchars(lePad2((string) $gameMainMin), ENT_QUOTES, 'UTF-8'); ?>&ndash;<?php echo htmlspecialchars(lePad2((string) $gameMainMax), ENT_QUOTES, 'UTF-8'); ?> for the selected window. The right panel shows the recency distribution: how many drawings ago each number last appeared.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values <?php echo htmlspecialchars(lePad2((string) $gameMainMin), ENT_QUOTES, 'UTF-8'); ?>&ndash;<?php echo htmlspecialchars(lePad2((string) $gameMainMax), ENT_QUOTES, 'UTF-8'); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. A lower recency value means the number appeared more recently.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the table for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. Numbers range from <?php echo htmlspecialchars(lePad2((string) $gameMainMin), ENT_QUOTES, 'UTF-8'); ?> to <?php echo htmlspecialchars(lePad2((string) $gameMainMax), ENT_QUOTES, 'UTF-8'); ?>.
        </p>
      </div>
    </div>
      ...
      <form name="fqsearch" method="post" action="/all-us-lotteries/results-analysis?st=<?php echo htmlspecialchars((string) ($stateAbrev ?? ''), ENT_QUOTES, 'UTF-8'); ?>&amp;stn=<?php echo htmlspecialchars((string) ($stateName ?? ''), ENT_QUOTES, 'UTF-8'); ?>&amp;gm=<?php echo htmlspecialchars((string) ($gName ?? ''), ENT_QUOTES, 'UTF-8'); ?>#tables">
        <div class="skai-controls-row">
          <div class="skai-controls-left">
            <label for="nod">Analysis draw window</label>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values <?php echo htmlspecialchars(lePad2((string) $gameMainMin), ENT_QUOTES, 'UTF-8'); ?>&ndash;<?php echo htmlspecialchars(lePad2((string) $gameMainMax), ENT_QUOTES, 'UTF-8'); ?></span>
          </div>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            ...
            <span class="skai-card-sub">Quick reference for the current <?php echo (int) $nodCurrentMain; ?>-draw window</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-summary-list">
              <div class="skai-summary-row">
                <span class="skai-summary-label">Top 5 most active</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(leCommaList(array_slice($topActiveKeys, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-summary-row">
                <span class="skai-summary-label">Top 5 quietest</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(leCommaList(array_slice($quietestKeys, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-summary-row">
                <span class="skai-summary-label">Repeated (trailing 10)</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(!empty($repeatedNumbers) ? leCommaList($repeatedNumbers) : 'None identified', ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-summary-row">
                <span class="skai-summary-label">50-draw leaders</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(leCommaList(array_slice($top50, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-summary-row">
                <span class="skai-summary-label">300-draw leaders</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(leCommaList(array_slice($top300, 0, 5)), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-summary-row">
                <span class="skai-summary-label">Latest draw date</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(leFmtDateLong($drawDate), ENT_QUOTES, 'UTF-8'); ?></span>
              </div>
              <div class="skai-summary-row">
                <span class="skai-summary-label">Numbers analyzed</span>
                <span class="skai-summary-value"><?php echo htmlspecialchars(lePad2((string) $gameMainMin), ENT_QUOTES, 'UTF-8'); ?>&ndash;<?php echo htmlspecialchars(lePad2((string) $gameMainMax), ENT_QUOTES, 'UTF-8'); ?> (<?php echo (int) $gameMainCount; ?> per draw)</span>
              </div>
            </div>

            <div class="skai-note" style="margin-top:16px">
              Use the table filter controls on the left to quickly narrow to most active, quietest, or recently seen numbers. Tracking is local to this page view and intended as a lightweight comparison aid.
            </div>
          </div>
        </div>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This page is designed to help users understand recent and historical behavior more clearly, not to imply certainty about future outcomes.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. The purpose of this page is to make the recent behavior of the game easier to understand, compare, and carry into deeper SKAI analysis. <strong>Frequency</strong> measures how often a number appeared in the selected window. <strong>Recency</strong> measures how many draws have passed since a number last appeared. <strong>Active numbers</strong> have appeared more often in the window. <strong>Quiet numbers</strong> have longer gaps since their last appearance. None of these signals predict future draws.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function renderCharts() {
    ...
    var topActiveCtx = safeCtx('topActiveChart');
    var quietCtx     = safeCtx('quietChart');
    var fullMainCtx  = safeCtx('fullMainChart');
    var recencyCtx   = safeCtx('recencyChart');

    if (topActiveCtx && chartData.topActiveLabels && chartData.topActiveLabels.length) {
    ...
    if (quietCtx && chartData.quietLabels && chartData.quietLabels.length) {
      new Chart(quietCtx, {
        type: 'bar',
        data: {
    ...
    if (fullMainCtx && chartData.mainLabels && chartData.mainLabels.length) {
    ...
    if (recencyCtx && chartData.mainLabels && chartData.mainLabels.length) {
